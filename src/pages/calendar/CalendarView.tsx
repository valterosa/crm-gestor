    ...
    switch (viewMode) {
      ...
      case 'week': {
        ...
        if (start.getMonth() === end.getMonth()) {
          ...
          return `${format(start, "d 'de' MMMM yyyy", { locale: ptBR })} - ${format(end, "d 'de' MMMM yyyy", { locale: ptBR })}`;
        }
      }
      case 'month':
      default:
        return format(selectedDate, "MMMM yyyy", { locale: ptBR }).charAt(0).toUpperCase() + format(selectedDate, "MMMM yyyy", { locale: ptBR }).slice(1);
    }
  };
          ...
          <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline">
                ...
                <span className="hidden md:inline">{format(selectedDate, 'dd/MM/yyyy')}</span>
              </Button>
            </PopoverTrigger>
  ...
  while (day <= endDate) {
    for (let i = 0; i < 7; i++) {
      ...
      days.push(
        ...
        >
          <div className={`text-right p-1 ${isToday ? 'font-bold text-uniga-blue' : ''}`}>
            {formattedDate}
          </div>
          <div className="overflow-y-auto max-h-[80px]">
            {dayEvents.map((event) => (
              <div
                key={event.id}
                className="mb-1 p-1 text-xs rounded bg-uniga-blue text-white truncate"
                title={event.titulo}
              >
  ...
  return (
    <div className="calendar-week overflow-x-auto">
      <div className="grid" style={{ gridTemplateColumns: '60px repeat(7, minmax(150px, 1fr))' }}>
        ...
        {days.map((day, index) => {
          const isToday = isSameDay(day, new Date());
          const dayEvents = events.filter(event => isSameDay(new Date(event.dataInicio), day));
          ...
          return (
            ...
            >
              ...
              <div className={`text-lg ${isToday ? 'text-uniga-blue font-bold' : ''}`}>
                {format(day, 'd')}
              </div>
            </div>
          );
              ...
              return (
                <div key={dayIndex} className="border h-12 border-l relative">
                  {hourEvents.map(event => (
                    <div
                      key={event.id}
                      className="absolute inset-x-0 bg-uniga-blue text-white rounded p-1 text-xs overflow-hidden z-10 mx-1"
                      style={{
                        top: '2px',
                        height: 'calc(100% - 4px)'
                      }}
...
const DayView = ({ date, events }: { date: Date, events: Evento[] }) => {
  ...
  // Eventos do dia
  const dayEvents = events.filter(event => isSameDay(new Date(event.dataInicio), date));
        ...
        {hours.map(hour => (
          <React.Fragment key={hour}>
            ...
            <div className="border-b h-12 relative">
              {/* Eventos desta hora */}
              {dayEvents
                .filter(event => new Date(event.dataInicio).getHours() === hour)
                .map(event => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className="absolute inset-x-0 bg-uniga-blue text-white rounded p-2 overflow-hidden z-10 mx-1"
                      style={{
                        top: '2px',
                    ...
                    >
                      ...
                      <div className="text-xs mt-1">{event.local || 'Sem local definido'}</div>
                    </div>
                  );
                })}
            </div>
