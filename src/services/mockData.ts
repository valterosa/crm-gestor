...
export const mockTarefas: Tarefa[] = [
  ...
  },
  {
    ...
    descricao: 'Preparar documentação de implementação para Transportes Rápidos',
    prazo: '2023-05-25T17:00:00.000Z',
    status: 'em_progresso',
    prioridade: 'media',
    responsavelId: '3',
...
export const getTarefasByStatus = (): Record<TaskStatus, Tarefa[]> => {
  ...
  mockTarefas.forEach(tarefa => {
    result[tarefa.status].push({
      ...
      lead: tarefa.leadId ? mockLeads.find(lead => lead.id === tarefa.leadId) : undefined
    });
  });
...
export const getEventosByData = (inicio: Date, fim: Date): Evento[] => {
  return mockEventos
    .filter(evento => {
      ...
      return (eventoInicio >= inicio && eventoInicio <= fim) || 
             (eventoFim >= inicio && eventoFim <= fim) || 
             (eventoInicio <= inicio && eventoFim >= fim);
    })
    .map(evento => ({
      ...evento,
      lead: evento.leadId ? mockLeads.find(lead => lead.id === evento.leadId) : undefined,
...
export const getEventosByUsuario = (userId: string): Evento[] => {
  return mockEventos
    .filter(evento => evento.participantes.includes(userId) || evento.criadoPor === userId)
    .map(evento => ({
      ...evento,
      lead: evento.leadId ? mockLeads.find(lead => lead.id === evento.leadId) : undefined,
      criadorUsuario: mockUsers.find(user => user.id === evento.criadoPor)
